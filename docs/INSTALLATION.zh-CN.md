...
在管理后台创建节点（或轮换 Token）后，弹窗会根据仅显示一次的 Token 生成完整
Linux 安装命令。命令会优先使用“站点设置”中的“Agent 连接地址”；该字段留空时
才回退到当前浏览器访问来源。点击“复制一键安装命令”，然后直接在被监控 VPS 运行
即可完成安装和连接。该命令包含 Token，使用前应按密钥妥善保管，避免粘贴到共享
终端、聊天记录或日志中。
