...
- Webhook、Telegram、Discord 与 SMTP 通知渠道，凭据加密存储
- 离线、CPU、带宽、周期流量和到期提醒，支持防抖、冷却、恢复通知、模板与发送历史
- 带密码和节点范围限制的只读图表分享
- 结构化徽章、外部链接及经过服务端净化的自定义 HTML
- 可自定义反向代理后的 Agent 连接地址、公开面板标题、头部与底部内容
- Ping/TCPing 探测目标可在目标编辑或节点编辑中直接分配，无需单独维护目标组
- 版本化配置导入/导出，支持预览和合并
- 使用口令加密的完整 SQLite 备份与可恢复的分阶段还原
- 密码登录与 GitHub OAuth 白名单登录；密码始终保留为安全回退
- 登录限流、验证码、OAuth state、CSRF、会话撤销、可信代理白名单及管理审计日志
...
在管理后台创建节点（或轮换 Token）后，弹窗会根据一次性 Agent Token 生成完整的
Linux 安装命令。命令会优先使用“站点设置”中的“Agent 连接地址”；留空时才回退到
当前浏览器访问来源。点击“复制一键安装命令”，然后直接在目标 VPS 运行即可完成安装
和连接。该命令包含 Token，使用前应按密钥妥善保管。
